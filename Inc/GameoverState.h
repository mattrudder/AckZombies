...
class CGameoverState : public CGameState<CGameoverState>
{
protected:
...
public:
	...
	CGameoverState() : m_poGameover(NULL), m_bInsert(true)
	{
	...
	*/
	virtual void onActivate(void)
	{
		...
		if (bFirstTime)
		{
			...
			m_poGameover = new CMenu(0, 0, pMM->getWindowWidth(), pMM->getWindowHeight(), 
				"../Textures/Menu/Gameover.tga", 1024, 768);
			m_poGameover->setType(MT_GAMEOVER);
			bFirstTime = false;
		}
		...
		// see if they have enough money to continue
		...
		m_poPlayer->m_oStats.computeFinalGrade(true);

        vector<CMenuButton*>::iterator iter = m_poGameover->getButtons().begin();

		while (iter != m_poGameover->getButtons().end())
		{
			delete (*iter);
			iter = m_poGameover->getButtons().erase(iter);
		}

		unsigned int nMoney = m_poPlayer->getAmtMoney();
		if (nMoney >= 50000)
		{
			pButton = new CContinueButton((nWindowWidth >> 1) - 128, (nWindowHeight >> 1) + 128, 
				256, 32, "../Textures/Menu/continue.tga", "../Textures/Menu/continueHover.tga", 256, 32, true, m_poGameover);
		}

		// standard buttons
		pButton = new CMainMenuButton((nWindowWidth >> 1) - 128, (nWindowHeight >> 1) + 165, 
			256, 32, "../Textures/Menu/mainmenubutton.tga", "../Textures/Menu/mainmenubuttonHover.tga", 256, 32, true, m_poGameover);

		pButton = new CQuitButton((nWindowWidth >> 1) - 128, (nWindowHeight >> 1) + 202, 
			256, 32, "../Textures/Menu/quit.tga", "../Textures/Menu/quitHover.tga", 256, 32, true, m_poGameover);
	}
